...
public class IocContainer:ASingletonBase<IocContainer>
{
    ...
    public void Register<T>(Type type, string key = "") => Register<T>(key,new Container(type));
    /// <summary>
    /// 接收一个参数为键值
    /// </summary>
    ...
    public void Register<T1,T2>(string key = "") where T1 : class where T2 : T1 => Register<T1>(key, new Container(typeof(T2)));
    /// <summary>
    /// 直接由实例祖册适合没有父类的容器;
    /// </summary>
    ...
    /// <param name="key"></param>
    public void Register<T>(T instance,string key = "") => Register<T>(key,new Container(instance.GetType(), instance));
